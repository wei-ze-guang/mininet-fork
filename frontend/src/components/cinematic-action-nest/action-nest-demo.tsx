...
export function ActionNestDemo() {
  ...
  return (
    <section className={`nest-stage ${rootExpanded ? "is-root-expanded" : ""} ${secondExpanded ? "is-second-expanded" : ""}`}>
      ...
      <div className="nest-header">
        <div>
          <div className="nest-kicker">演示效果 / 无限套娃动作</div>
          <h2 className="nest-title">数据到达组件，组件展开，完成后缩回</h2>
          <p className="nest-copy">
            这个演示不绑定真实网络数据，只验证一种镜头语法：每个动作都可以成为下一层动作的容器。
          </p>
        </div>
      ...
      <div className="nest-scene">
        ...
        {stages.map((stage, index) => (
          <ActionNode key={stage.id} index={index} stage={stage} />
        ))}
